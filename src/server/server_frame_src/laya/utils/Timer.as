package laya.utils {
	...
	public class Timer {
		...
		private static const INTERVAL: int = 16;
		...
		/** 当前时间。*/
		public var currTimer:Number = NodeJS.now();
		/** 当前的帧数。*/
		public var currFrame:int = 0;
		/**@private */
		private var _lastTimer:Number = NodeJS.now();
		/**@private */
		...
		public function Timer() {
			Laya.timer ? Laya.timer.frameLoop(1, this, _update) : NodeJS.setInterval(_update.bind(this), INTERVAL);
		...
		public function _create(useFrame:Boolean, repeat:Boolean, delay:int, caller:*, method:Function, args:Array, coverBefore:Boolean):void {
			...
			if (coverBefore) {
				...
				if (handler) {
					...
					handler.args = args;
					handler.exeTime = delay + (useFrame ? this.currFrame : this.currTimer+NodeJS.now()-_lastTimer);
					return;
				}
			}
		...
		private function _indexHandler(handler:TimerHandler):void {
			...
			var cid:int = caller ? caller.$_GID || (caller.$_GID = getGID()) : 0;
		...
		public function clear(caller:*, method:Function):void {
			var handler:TimerHandler = _getHandler(caller, method);
			if (handler) {
				_map[handler.key] = null;handler.key = 0;
				handler.clear();
			}
		}
